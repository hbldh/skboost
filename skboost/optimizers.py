...
def perform_search(f, bounds, desired_value=0.0, x_tol=1e-5):
    ...
    logger.debug("Start interval = [{0} {1}], Values = [{2} {3}].".format(l_point, r_point, l_point_val, r_point_val))
    ...
    if l_point_val < desired_value:
        comp_fcn = np.less
        logger.debug("Assuming increasing evaluation function. " "Using {0} for comparisons".format(str(comp_fcn)))
    else:
        comp_fcn = np.greater
        logger.debug("Assuming decreasing evaluation function. " "Using {0} for comparisons".format(str(comp_fcn)))
    ...
    while np.abs(r_point - l_point) > x_tol:
        ...
        logger.debug(
            "New interval = [{0} {1}], {2}{3} {4}].".format(l_point, r_point, "Values = [", l_point_val, r_point_val)
        )
        logger.debug("Interval span = {0}, Stop criterion = {1}.".format(np.abs(r_point - l_point), x_tol))

    best_point = np.mean([l_point, r_point])
    logger.debug("Found x = {0} as best point, f(x) = {1}.".format(best_point, f(best_point)))
...
def golden_section_search(f, bounds, x_tol=1e-5):
    ...
    gm = (1 + np.sqrt(5)) / 2
    points = np.array(
        [bounds[0], bounds[1] - (bounds[1] - bounds[0]) / gm, bounds[0] + bounds[1] / gm, bounds[1]], "float"
    )
    ...
    while np.abs(points[3] - points[0]) > x_tol:
        ...
        if cmp_fcn(vals[1], vals[2]):
            points = np.array([points[0], points[2] - (points[2] - points[0]) / gm, points[1], points[2]], "float")
            vals = np.array([vals[0], new_val, vals[1], vals[2]])
        else:
            points = np.array([points[1], points[2], points[1] + ((points[3] - points[1]) / gm), points[3]], "float")
            vals = np.array([vals[1], vals[2], new_val, vals[3]])
        logger.debug("New interval = {0}, Values = {1}.".format(points, vals))
        logger.debug("Interval span = {0}, Stop criterion = {1}.".format(np.abs(points[3] - points[0]), x_tol))

    best_ind = end_eval_fcn(vals)
    best_point = points[best_ind]
    logger.debug("Found x = {0} as best point, f(x) = {1}.".format(best_point, vals[best_ind]))
